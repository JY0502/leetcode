...
BinaryHeap.prototype.insert = function (value) {
  ...
  const switching = (currIdx) => {
    if (currIdx === 0) {
      return;
    }
    const parentIdx = Math.floor((index - 1) / 2);
    if (this._heap[currIdx] < this._heap[parentIdx]) {
      const currVal = this._heap[currIdx];
      this._heap[currIdx] = this._heap[parentIdx];
      this._heap[parentIdx] = currVal;
      switching(Math.floor((index - 1) / 2));
    }
  };
  switching(this._heap.length - 1);
};
...
BinaryHeap.prototype.removeRoot = function () {
  ...
  this._heap.pop();
  const comparing = (currIdx) => {
    if (!this._heap[currIdx * 2 + 1]) {
      return;
    }
    let childIdx = index * 2 + 1;
    if (this._compare(this._heap[index * 2 + 2], this._heap[index * 2 + 1])) {
      childIdx = index * 2 + 2;
    }
    if (this._compare(childIdx, currIdx)) {
      const currVal = this._heap[currIdx];
      this._heap[currIdx] = this._heap[childIdx];
      this._heap[childIdx] = currVal;
      comparing(childIdx);
    }
  };
  comparing(0);
